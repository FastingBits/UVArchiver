...
async function testRBAC() {
  ...
  let depts = await query('SELECT id_departamento FROM departamentos WHERE nombre = ? AND deleted_at IS NULL', ['Depto Auditoria y Calidad']);
  let deptId;
  if (depts.length === 0) {
    const res = await query('INSERT INTO departamentos (nombre, descripcion) VALUES (?, ?)', ['Depto Auditoria y Calidad', 'Pruebas']);
    deptId = res.insertId;
  } else {
  ...
  let depts2 = await query('SELECT id_departamento FROM departamentos WHERE nombre = ? AND deleted_at IS NULL', ['Depto Secundario QA']);
  let deptId2;
  if (depts2.length === 0) {
    const res2 = await query('INSERT INTO departamentos (nombre, descripcion) VALUES (?, ?)', ['Depto Secundario QA', 'Pruebas 2']);
    deptId2 = res2.insertId;
  } else {
  ...
  async function upsertUser(email, nombre, rol, dept, inst) {
    const existing = await query('SELECT id_usuario FROM usuarios WHERE correo = ?', [email]);
    if (existing.length > 0) {
      await query('UPDATE usuarios SET nombre=?, id_rol=?, id_departamento=?, pertenece_a_institucion=?, contrasena=?, deleted_at=NULL WHERE id_usuario=?',
        [nombre, rol, dept, inst, passwordHash, existing[0].id_usuario]);
      return existing[0].id_usuario;
    } else {
      const res = await query('INSERT INTO usuarios (nombre, apellido_paterno, apellido_materno, correo, contrasena, id_rol, id_departamento, pertenece_a_institucion) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
  ...
  console.log('--- TESTING AUTH SESSIONS ---');
  ...
  results.push({ test: 'SysAdmin no puede iniciar sesión', status: sysInvalidLogin.statusCode === 401 ? 'PASS' : 'FAIL', code: sysInvalidLogin.statusCode });
  results.push({ test: 'Admin no puede iniciar sesión', status: adminInvalidLogin.statusCode === 401 ? 'PASS' : 'FAIL', code: adminInvalidLogin.statusCode });
  results.push({ test: 'User no puede iniciar sesión', status: userInvalidLogin.statusCode === 401 ? 'PASS' : 'FAIL', code: userInvalidLogin.statusCode });
  results.push({ test: 'Auditor no puede iniciar sesión', status: auditorInvalidLogin.statusCode === 401 ? 'PASS' : 'FAIL', code: auditorInvalidLogin.statusCode });
  ...
  if (addAdminCrossDept.body?.userId) {
    const created = await query('SELECT id_departamento FROM usuarios WHERE id_usuario = ?', [addAdminCrossDept.body.userId]);
  ...
  results.push({
    test: 'Usuario en Sin Dept (1) no puede tener rol de Admin (2) incluso cuando es actualizado por SysAdmin (400)',
  ...
  // TEST 3: Documents Module & Supervision View
  console.log('\n--- 4. DOCUMENTOS ---');
  // Read own documents
  ...
  results.push({ test: 'SysAdmin puede acceder a la vista de supervisión /documentos/departamento/:id', status: sysSupervision.statusCode === 200 ? 'PASS' : 'FAIL', code: sysSupervision.statusCode });
  results.push({ test: 'Admin no puede acceder a la vista de supervisión /documentos/departamento/:id (403)', status: adminSupervision.statusCode === 403 ? 'PASS' : 'FAIL', code: adminSupervision.statusCode });
  results.push({ test: 'User no puede acceder a la vista de supervisión /documentos/departamento/:id (403)', status: userSupervision.statusCode === 403 ? 'PASS' : 'FAIL', code: userSupervision.statusCode });
  results.push({ test: 'Auditor no puede acceder a la vista de supervisión /documentos/departamento/:id (403)', status: auditorSupervision.statusCode === 403 ? 'PASS' : 'FAIL', code: auditorSupervision.statusCode });
  ...
  const passedPercentage = (passed / length) * 100;
  const failedPercentage = (failed / length) * 100;
  console.log('Total de pruebas: ', length);
  console.log(`Porcentaje de pruebas pasadas: ${passedPercentage.toFixed(2)}%`);
  console.log(`Porcentaje de pruebas fallidas: ${failedPercentage.toFixed(2)}%`);
